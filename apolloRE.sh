...
Options:
  ...
  -m, --mode MODE           passive | web | full (default: full)
      --modules LIST        Comma-separated modules (subdomains,http,dns,ports,crawl,javascript,nuclei,screenshots,report)
      --rate-limit N        Requests/second hint for supported tools (default: 50)
      --resume              Skip stages whose expected output already exists
  -o, --output DIR          Base results directory (default: ./results)
...
Examples:
  ...
  ./apolloRE.sh -d example.com --modules subdomains,http,dns,report --resume
...
export ROOT_DOMAIN MODE RESUME VERBOSE RATE_LIMIT
