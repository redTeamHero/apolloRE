...
run_http() {
  ...
  "$httpx_cmd" -silent -l "$subs" -rl "$RATE_LIMIT" -o "$alive" || { log_warn "httpx probe failed"; return 0; }
  "$httpx_cmd" -silent -l "$subs" -rl "$RATE_LIMIT" -json -status-code -title -tech-detect -web-server -ip -o "$meta" 2>/dev/null || true
}
